...
import 'src/media_track.dart';
export 'src/media_track.dart';
import 'src/video_player_platform_interface.dart';
export 'src/video_player_platform_interface.dart' show DurationRange, DataSourceType, VideoFormat, VideoPlayerOptions;
...
class VideoPlayerController extends ValueNotifier<VideoPlayerValue> {
  ...
  /// All supported mediaTracks in the current video.
  Future<List<MediaTrack>> get mediaTracks async {
    if (_isDisposed) {
      return Future.value([]);
    }
    List<MediaTrack> _tracks =[];
    await _videoPlayerPlatform.getMediaTracks(_textureId).then((value)async{
      if(value != null){
        var rawData = jsonDecode(value);
        int rendererIndex = rawData["rendererIndex"];
        int selectedHeight = rawData["selectedHeight"];
        var _mediaTracks = jsonDecode(rawData["tracks"]);
        if(selectedHeight != null) {
          _tracks.add(MediaTrack(
            ...
            trackIndex: -1,
            bitrate: 0,
            selected: rendererIndex < 0,
            height: rendererIndex < 0 ? selectedHeight : null,
          ));

          _tracks.addAll(MediaTrack.listFromJson(_mediaTracks)
              .map((e) => e..selected = rendererIndex >= 0 && e.height == selectedHeight)
              .toList());
        }
      }
    }).catchError((e)async{
  ...
  ///Change media track
  /// Renderer = -1 is treated as Auto quality.
  Future<void> setMediaTrack(MediaTrack mediaTrack) async {
    if(mediaTrack != null){
      await _videoPlayerPlatform.setMediaTrack(_textureId,mediaTrack);
    }
  }
